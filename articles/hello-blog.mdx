...
I've been thinking about creating a blog with **Next.js** and **MDX** for a while now. I've read a lot of articles about it, but honestly I haven't found any good ones. So I decided to create my own blog and write an article about it.
...
I don't want to explain how to create a new app using Next.js so I'll assume you already know how to do that.
...
This packages allows you load MDX files from anywhere, in this case I'll retrieve the data from the server using Node.

## ⚙️ The Parser

In this project I created a module called `parser` to fetch, resolve, sort and compile all the MDX files. With the parser I'll prepare all the _frontmatter_ with the data.
...
The `getArticleSlugs` function returns an array with all the slugs of the articles reading from the filesystem and looking for MDX files inside the `./articles` path.
...
The `matter` method will parse the _front-matter_ from the article and it's content, it will return the **data** (that contains the "header" part) and the **content** (that will contain the MDX content).
Finally we prepare our final object with the data, composing other elements useful for the frontend such as the **reading-time**, the **permalink**, the **date**, etc.
...
The `serialize` method consumes a string of _MDX_. It will return an object that will contains the compiled source that will be passed to the `<MDXRemote>` component.
...
Ok so we are just asking to my server for all the articles and we will return them to the client.
...
As you can see having all the data inside my props I can simply map the articles data into a `Preview` component.
...
Keep calm bro, have you copied and pasted the code? It's working? Well, it's not the end of the world. I spent 3 days to get it working.
...
Ok ok ok... What the hell is **MDXRemote**? It's a component that consumes the _serialized_ compiled source and use the _components_ you passed to it. Each component will render only in your **MDX** source. In my case I prefer create specific components for each case.
...
I advise you to use the typography plugin. It helps you to create a good typography and layout for your posts.
...
The reason I hide the **before** and **after** mutators it's because the _typography_ plugin appends the **\`** char into the `<code>` element.
