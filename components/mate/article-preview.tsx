...
export function ArticlePreview({ author, date, title, description, image, slug, tags }: ArticlePreviewProps) {
  return (
    <Card className="w-full bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 transition-all duration-300 hover:shadow-md">
      <Link href={`/blog/${slug}`} className="block">
        <div className="relative h-[180px] sm:h-[220px] lg:h-[280px] w-full overflow-hidden rounded-t-xl">
          ...
          <div className="absolute inset-x-0 bottom-0 z-20 p-4 sm:p-6">
            <CardTitle className="line-clamp-2 text-lg sm:text-xl lg:text-2xl text-white">{title}</CardTitle>
          </div>
        </div>
      </Link>
